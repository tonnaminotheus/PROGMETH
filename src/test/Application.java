			...
			if (choice.equals("1")) {
				...
				if (GameController.get_turn() % 2 == 1) {
					...
					while (true) {
						...
						boolean t1 = GameController.getCurrentMap().getEntity((posx + x) / 2, (posy + y) / 2)
								.is_WhiteTile();
				...
				else if (GameController.get_Player2().getHaveBaricade() == 0) {
					System.out.println("you dont have any barricade left");
					continue;
				} else
					...
					if (ch.equals("1")) {
						if (GameController.addHorisontalBarricade(x, y)) {
							if (!GameController.checkbfs(
									GameController.get_turn() % 2 == 1 ? GameController.get_Player1()
											: GameController.get_Player2(),
									GameController.get_turn() % 2 == 1 ? 16 : 0)) {
