...
public class GameController {
	...
	public static boolean move(Player player,int x,int y)
	{
		...
		//this.setX(x);   +2
		//this.setY(y);   +2
		if(getCurrentMap().getEntity(x, y).is_BlackTile() 
				||getCurrentMap().getEntity(x, y).is_SpecialTile()) {
			int xx=player.getX();
			int yy=player.getY();
			player.setX(x);
			player.setY(y);
	...
	{
		Entity e1=getCurrentMap().getEntity(x, y);
		...
		if(e1.getClass()==WhiteTile.class && e2.getClass()==WhiteTile.class)
		{
			...
			getCurrentMap().addEntity(new BarricadeTile(x,y,x,y+1), x, y);
			getCurrentMap().addEntity(new BarricadeTile(x,y+1,x,y), x, y+1);
			return true;
		}
		else
		{
	...
	public static boolean addHorisontalBarricade(int x,int y)
	{
		...
		if(e1.getClass()==WhiteTile.class && e2.getClass()==WhiteTile.class)
		{
			getCurrentMap().removeEntity(e1.getX(), e1.getY());
			getCurrentMap().removeEntity(e2.getX(), e2.getY());
			getCurrentMap().addEntity(new BarricadeTile(x,y,x+1,y), x, y);
			getCurrentMap().addEntity(new BarricadeTile(x+1,y,x,y), x+1, y);
			return true;
		}
		else
		{
	...
	public static boolean checkbfs(Player player,int finish)
	{
		...
		while(!q.isEmpty())
		{
			...
			for(int i=0;i<4;i++)
			{
				if(0<=aa.getX()+dirx[i]&& aa.getX()+dirx[i]<17  && 0<=aa.getY()+diry[i]&& aa.getY()+diry[i]<17 )
				{
					...
					boolean t3 = getCurrentMap().getEntity(aa.getX()+dirx[i]/2, aa.getY()+diry[i]/2).is_WhiteTile();
